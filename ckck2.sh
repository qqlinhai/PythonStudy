...
verify_ck(){
    ...
    for i in ${!value[@]}; do
        ...
        if [[ $NOTIFY_WSKEY_NO_EXIST = 1 || $NOTIFY_WSKEY_NO_EXIST = 2 ]]; then
            ...
            [[ $WSKEY_REMARK_SYNC = 1 ]] && [[ ${wskey_value[i]} && ${wskey_value[i]} != null ]] && [[ ${remarks_id[i]} != ${wskey_remarks[i]} ]] && ql_update_env_api JD_WSCK "${wskey_value[i]}" ${wskey_id[i]} "${remarks_id[i]}"
        fi
